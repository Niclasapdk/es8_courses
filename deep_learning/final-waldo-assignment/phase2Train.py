...
IMG_SRC = "/kaggle/working/images_combined"   # was resized_224x224
LBL_SRC = "/kaggle/working/labels_combined"   # was labels_resized_224x224
...
for split in ["train", "val", "test"]:
    ...
    empty = sum(1 for f in files if os.path.getsize(os.path.join(lbl_dir, f)) == 0)
    print(f"{split}: {len(files)} labels, {empty} negatives ({100*empty/len(files):.1f}%)")
